...
def test_sunrise_ramps_up_monotonically():
    e = LightEngine(NATURAL_DAY)  # start 08:00, sunrise 60m
    b0 = _at(e, 8, 0).brightness
    b15 = _at(e, 8, 15).brightness
    b45 = _at(e, 8, 45).brightness
    assert b0 == 0
    assert b0 < b15 < b45
    assert _at(e, 8, 15).phase is Phase.SUNRISE


def test_peak_holds_max_intensity():
    e = LightEngine(NATURAL_DAY)  # peak from 09:00 to 15:00, max 80
    s = _at(e, 12, 0)
...
def test_sunset_ramps_down():
    e = LightEngine(NATURAL_DAY)  # sunset 15:00->16:00
    b15 = _at(e, 15, 0).brightness
    b1530 = _at(e, 15, 30).brightness
    b16 = _at(e, 16, 0).brightness
    assert b15 == 80
    assert b15 > b1530 > b16
    assert _at(e, 15, 30).phase is Phase.SUNSET


def test_moonlight_window_active_then_night():
    e = LightEngine(NATURAL_DAY)  # day ends 16:00, moonlight 120m -> 18:00
    moon = _at(e, 17, 0)
...
def test_channels_scale_with_brightness():
    e = LightEngine(NATURAL_DAY)
    mid = _at(e, 8, 30)  # partway up the sunrise ramp
